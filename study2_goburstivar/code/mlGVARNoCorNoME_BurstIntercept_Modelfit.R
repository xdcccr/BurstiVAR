...
# Original file: paper_simulation_archive/Study2_Gompertz/code/mlGVARNoCorNoME_BurstIntercept_Modelfit.R
...
# mlGVARNoCorNoME_BurstIntercept_5Burst.txt, posteriorSummaryStats.R and
# ./data here, and writes to ./result. The nT_list/nP_list headers are
...
# ==============================================================================
# mlGVARNoCorNoME_BurstIntercept_5Burst_Modelfit.R
# 功能：用5-Burst Intercept模型拟合GompertzBurst数据
# ==============================================================================
rm(list=ls())
library(parallel)
...
find_missing <- function(base_path, nT, nP, N_repl) {
  # C_data: 数据来源（GompertzBurst）
  # 结果文件名使用BurstIntercept以区分拟合模型
  C_result <- "mlGVARNoCorNoME_BurstIntercept_5Burst"
...
run_one_replication <- function(r, base_path, nT, nP) {
  ...
  # 拟合模型：BurstIntercept（5 burst版本）
  C_fit <- "mlGVARNoCorNoME_BurstIntercept_5Burst"
  # 结果命名：使用BurstIntercept以区分
  C_result <- "mlGVARNoCorNoME_BurstIntercept_5Burst"
