...
directory. This page itself is knitted from `tutorial.Rmd`, so every output
block below is the real output of the code above it, produced during the
knit. You need R (tested with 4.3.2), JAGS (tested with 4.3.1), and the R
...
# Knit-time display option only: suppress the MCMC progress bars, which do
# not render well in a static document. No modeling setting is changed.
...
The chunk below prints the summary next to the generating values (posterior
mean, posterior SD, 95% equal-tailed credible interval, ESS, and R-hat, with
the generating value in the `true` column). Because this document is knitted,
the numbers are the actual output of the fit above:
